...
for aug_model in $(ls augment_models); do
    augment_lang=${aug_model: -6:3}
    for lang in $(ls data); do
        ...
        if [ "$lang" = "$augment_lang" ]; then
            ...
            for src in $(ls data/$lang); do
                ...
                # int a new bitext.
                ~/joshua-scripts/weight_tables.py $PWD/data/$lang/$src/trn.eng $PWD/data/$lang/$src/trn.$lang augment_models/$aug_model $norm_constant
                ...
                mkdir $new_folder
                mv ${norm_constant}x_combined_.$src.$lang $new_folder/trn.$lang
                mv ${norm_constant}x_combined_.$src.eng $new_folder/trn.eng
