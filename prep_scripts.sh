...
for lang in $(ls ./data); do
    ...
    for tst in $(ls ./runs/$lang); do
        ...
        for src in $(ls ./data/$lang); do
            base_target=qsub_scripts/base_$tst.$src.$lang.qsub
            ...
            echo "$JOSHUA/bin/pipeline.pl --rundir $count --readme 'Baseline Run lang:$lang train:$src test:$tst' --source $lang --target eng --type hiero --corpus $PWD/data/$lang/$src/trn --tune $PWD/data/$lang/$src/dev --test $PWD/runs/$lang/$tst/tst --maxlen 80 --lm-order 3" >> $base_target
            count=$(($count + 1))
            ...
            echo "$JOSHUA/bin/pipeline.pl --rundir $count --readme 'System Run lang:$lang train:$src test:$tst' --source $lang --target eng --type hiero --corpus $PWD/data/$lang/$src/trn --tune $PWD/data/$lang/$src/dev --test $PWD/runs/$lang/$tst/tst --maxlen 80 --lm-order 3 --joshua-config $PWD/inputs/joshua.config.$lang" >> $augment_target
            count=$(($count + 1))
        done
    done
done
